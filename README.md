...
A basic Tkinter GUI is available for batch converting every `.nbt` or `.snbt` file in a folder. Each converted file is written back to the same folder with the chosen `.mcfunction` or `.txt` extension, and you can optionally merge everything into one combined `.txt` after conversion.
...
3. Pick the output type (`.mcfunction` or `.txt`), toggle the tellraw/chunking options, and (optionally) enable merging into a single combined `.txt` that separates structures with blank lines and a `NEXT STRUCTURE` marker.
4. Click **Convert** to write the files alongside each source file. Progress is reported in the on-screen log and a summary dialog; if merging is enabled a `combined_structures.txt` file will also be created in the same folder.
...
* **Save as** – Choose whether per-file outputs are written with the `.mcfunction` or `.txt` extension.
* **Merge all outputs into combined .txt with separators** – After conversion, write a `combined_structures.txt` file that concatenates every generated file, separated by blank lines, a `NEXT STRUCTURE` line, and another blank line.
