...
- Node.js and npm still need to be installed on your system; downloading the repo does not bundle Node itself. On Windows, if `npm` is missing, the GUI attempts to download a portable Node.js+`npm` bundle automatically and use that for installation.
...
- When `npm` is missing, a portable Node.js+`npm` bundle is downloaded to `embedded_node/` next to `converter_gui.py` and used automatically.
