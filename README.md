...
- If a local binary cannot be located after install, conversions fall back to `npm --prefix nbt-to-mcstructure exec --yes nbt-to-mcstructure ...`, using your available npm (or the embedded copy) instead of relying on `npx` being on PATH. This keeps conversions working even when the repository does not expose a `.bin` shim.
...
### Windows notes
- The GUI prefers the locally installed `node_modules/.bin/nbt-to-mcstructure.cmd` that `npm install` creates. If that binary cannot be used, it falls back to `npm --prefix nbt-to-mcstructure exec --yes nbt-to-mcstructure ...`, so only `npm` needs to be on PATH.
